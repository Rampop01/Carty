...
console.log("   select 1      — Choose option 1");
console.log("   /cancel       — Cancel the current session");
console.log("");
...
for await (const [space, message] of app.messages) {
  await space.responding(async () => {
    try {
      ...
      // Check if it's a command like /shop or /cancel
      ...
      // If not a command, check if it's a selection or fulfillment choice
