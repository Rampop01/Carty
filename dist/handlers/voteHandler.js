 ...
 */
function parseVote(text, session) {
    const lower = text.toLowerCase().trim();
    // Match "vote 1", "I vote 2", "put me down for 3", etc.
    if (lower.includes("vote 1") || lower === "1" || lower.includes("vote a"))
        return 0;
    if (lower.includes("vote 2") || lower === "2" || lower.includes("vote b"))
        return 1;
    if (lower.includes("vote 3") || lower === "3" || lower.includes("vote c"))
        return 2;
...
export async function handleVote(chatId, senderId, senderName, text, send) {
    ...
    if (voteIndex >= session.products.length)
        return false;
